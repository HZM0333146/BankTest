...
import idv.hzm.app.aop.BankAuthenticate;
import idv.hzm.app.aop.BankAuthenticateType;
import idv.hzm.app.aop.BankMember;
import idv.hzm.app.aop.Member;
...
public class CoinController {
	...
	@BankAuthenticate(types = { BankAuthenticateType.MEMBER, BankAuthenticateType.OTP, BankAuthenticateType.VISITOR })
	@GetMapping(value = "/coins/{a}/{b}", produces = MediaType.APPLICATION_JSON_VALUE)
	@ApiOperation(value = "取得所有錢幣", notes = "")
	@ApiResponses(value = { @ApiResponse(code = 200, message = "成功", response = String.class) })
	public ResponseEntity<List<Coin>> getAllCoin(@BankMember Member member,@PathVariable("a") String a,
